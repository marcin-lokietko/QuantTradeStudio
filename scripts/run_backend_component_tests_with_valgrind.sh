...
export BACKEND_CMD="valgrind --leak-check=yes /algo-trader/build/debug/src/AlgoTrader /algo-trader/tests/backend/configuration.json > /algo-trader/logs/valgrind/valgrind_output.txt 2>&1"
...
VALGRIND_PID_IN_DOCKER=$(docker exec ${BACKEND_CONTAINER_ID} ps aux | grep 'valgrind.bin.*AlgoTrader' | awk '{print $2}')
